...
public class DefaultClient extends AbstractClient {
	...
	protected Response sendXmlRequest(URL url, RequestMethod method, String xml) throws IOException, OAuthException {
		return sendRequest(url, method, xml, "application/xml");
	}
	 ...
	 */
	@Override
	protected Response sendJsonRequest(final URL url, final RequestMethod method, final String json) throws IOException, OAuthException {
		return sendRequest(url, method, json, "application/json");
	}

	private Response sendRequest(final URL url, RequestMethod method, String request, final String applicationType) throws IOException, OAuthException {
		method = method == null ? RequestMethod.GET : method;

		request = (RequestMethod.POST.equals(method) || RequestMethod.PUT.equals(method)) ? StringUtils.trimToNull(request) : null;
		...
		try {
			...
			connection.setRequestMethod(method.name());
			if (request != null) {
				connection.setRequestProperty("Content-Type", applicationType + "; charset=" + getEncoding().toLowerCase());
				connection.setRequestProperty("Content-Length", String.valueOf(request.getBytes(getEncoding()).length));
				connection.setRequestProperty("Content-Language", "en-US");
			}
			connection.setRequestProperty("Accept", applicationType);
			connection.setUseCaches(false);
			connection.setDoInput(true);
			connection.setDoOutput(true);
			getAuthConsumer().sign(connection);
